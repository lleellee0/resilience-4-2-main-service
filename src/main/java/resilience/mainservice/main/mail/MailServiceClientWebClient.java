...
//    @RateLimiter(name = MAIL_SERVICE_RL, fallbackMethod = "sendMailFallback")
     ...
     */
    public String sendMailFallback(EmailRequest emailRequest, Throwable t) {
        // 어떤 예외로 Fallback이 실행되었는지 로깅 (RateLimiter, Bulkhead 예외 포함)
        log.warn("[Resilience Fallback] Fallback activated for email: {}. Reason: {}",
                emailRequest.getEmail(), t.getClass().getSimpleName() + ": " + t.getMessage());

        // Fallback 로직
        // 유저에 의한 호출이었다면, 추상적인 예외를 만들어서 클라이언트에게 횟수 제한을 초과했다던지 알려주면 좋음.
        throw new RuntimeException("Mail service temporarily unavailable or limit exceeded. Fallback executed for " + emailRequest.getEmail() + ". Cause: " + t.getMessage(), t);
    }
}